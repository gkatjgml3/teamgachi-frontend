...
- 2026-08-20 타이머를 실제 경과 시각 기준으로 계산해 다른 화면을 다녀와도 계속 진행되게 하고, 시작부터 사용자가 종료할 때까지 증가하는 `시간 기록` 모드 추가
- 2026-08-20 사용자별로 고정된 랜덤 파스텔 프로필 색상을 배정하고 채팅·대시보드·진척도·타이머·공지사항에 동일한 색상으로 표시
- 2026-08-20 상단 알림 버튼에 새 알림 개수와 읽지 않은 공지·임박한 할 일·일정 목록을 연결하고 새 공지 브라우저 알림 기능 추가
- 2026-08-20 팀 안에서 프로필 파스텔 색상이 서로 겹치지 않도록 고유 배정하고, 채팅 연속 전송 잠금과 Realtime 갱신 중복 방지 추가
- 2026-08-20 대시보드의 할 일 추가 버튼을 상세 내용·담당자·마감 시간·공동 작업자·증빙 설정이 있는 동일한 할 일 등록 폼으로 연결
- 2026-08-20 월간 캘린더에서 할 일과 일정 제목이 날짜 숫자 옆이 아닌 아래쪽에 정렬되도록 수정
- 2026-08-20 타이머 상태 문구를 원 밖 아래로 이동하고 `오늘 완료 0회` 세션 횟수 표시 제거
- 2026-08-20 팀장이 팀 관리에서 팀원별 공지 작성 권한을 허용·해제할 수 있도록 DB 권한과 공지 화면 연결
- 2026-08-20 채팅 첨부 파일을 채팅 메시지에만 저장·표시하고, 메시지의 `드라이브에 추가` 버튼을 눌러야 자료함에 등록되도록 분리
- 2026-08-20 공지 클릭 시 작성자·작성일·구분 표를 숨기고 공지 제목과 핵심 본문만 표시하도록 상세 화면 단순화
- 2026-08-20 공지 HTML의 샘플 UI가 로딩 중 노출되지 않도록 강제 숨김하고 공지 JavaScript 캐시 버전 갱신
- 2026-08-19 자료 드라이브의 `전체` 개수가 현재 탭이 아닌 파일·링크·이미지 전체 자료 수를 표시하도록 수정
- 2026-08-19 자료함의 앱·DB·버킷 자체 파일 크기 제한을 제거하고 화면에는 실제 사용량만 표시하도록 수정(Supabase 프로젝트 한도 적용)
- 2026-08-19 채팅의 AI 대화 요약을 상단에 고정하고 메시지 목록만 독립적으로 스크롤되도록 수정
- 2026-08-19 전 화면 로고와 파비콘을 새 보라색 팀가치 심볼로 교체하고, D-Day를 시간 차이가 아닌 한국 날짜 기준으로 계산하도록 수정
